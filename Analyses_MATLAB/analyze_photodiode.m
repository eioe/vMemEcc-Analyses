...
EEG = pop_epoch( EEG, {'S 23'}, [-0.1 0.3], 'newname', '70hz epochs', 'epochinfo', 'yes');
eeglab redraw;
erpel = mean(EEG.data(1,:,:), 3);
plot([1:1000/EEG.srate:400], erpel);

starts = find(strcmp('S 23', {EEG.event.type}));
ends = find(strcmp('S 24', {EEG.event.type}));
slat = [EEG.event(starts).latency];
elat = [EEG.event(ends).latency];
trigDist = mean(elat-slat);

hold on
vline([100 100+trigDist*1000/EEG.srate], {'g','r'}, {'StimOn' 'StimOff'})