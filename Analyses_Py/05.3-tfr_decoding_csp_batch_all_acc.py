...
for shuf_labels_bool in [False]:

    _ = decode(sub_list_str, ['LoadLowEccL', 'LoadHighEccL'], config.event_dict, reps=100, scoring='accuracy', 
               shuffle_labels=shuf_labels_bool, overwrite=True)
    _ = decode(sub_list_str, ['LoadLowEccS', 'LoadHighEccS'], config.event_dict, reps=100, scoring='accuracy', 
               shuffle_labels=shuf_labels_bool, overwrite=True)
    _ = decode(sub_list_str, ['LoadLowEccM', 'LoadHighEccM'], config.event_dict, reps=100, scoring='accuracy', 
               shuffle_labels=shuf_labels_bool, overwrite=True)
    _ = decode(sub_list_str, ['LoadLow', 'LoadHigh'], config.event_dict, reps=100, scoring='accuracy', 
           shuffle_labels=shuf_labels_bool, overwrite=True)
