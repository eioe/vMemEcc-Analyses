...
save_data(filtered, subsub, path_outp_filt, append='-filt') 
        

## filtered = load_data_raw(subsub+'-filt', path_outp_filt)

# Define relevant events:
targ_evs_orig = [i for i in np.arange(150, 174)]
targ_evs = [event_id['Stimulus/S%03d' % ss] for ss in targ_evs_orig]
...
trig_postcal = event_id['Stimulus/S228']

for i in range(len(events)):
    if events[i,2] in targ_evs and events[i+2,2] == trig_postcal:
        events[i:i+2,2] = 999


# crop off all markers before first and after last exp block:
trig_b1_start = event_id['Stimulus/S208']
trig_b10_end = event_id['Stimulus/S247']
rel_evs = events[:,2]
idx_start = np.where(rel_evs == trig_b1_start)[0][0]
idx_end = np.where(rel_evs == trig_b10_end)[0][0]
events = events[idx_start:idx_end+1,:]

# set up event arrays:
events_fix = np.array([ev for ev in events if ev[2] in targ_evs])
# add duration of blinking interval:
# FIXME: replace 800 with variable 
events_fix[:,0] = events_fix[:,0] + 800*filtered.info['sfreq']/1000

events_cue = np.array([ev for ev in events if ev[2] == event_id['Stimulus/S  1']])
events_stimon = np.array([ev for ev in events if ev[2] == event_id['Stimulus/S  2']])
events_stimon[:,2] = events_fix[:,2]


epoStimOn = mne.Epochs(filtered, 
                        events_stimon, 
                        event_id=None, 
                        tmin=-0.5, 
                        tmax=2.2, 
...
save_data(epoStimOn, subsub + '-stimOn', path_outp_epo, '-epo')
