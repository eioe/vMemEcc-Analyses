    ...
    for cond in conds:
        ...
        for sub in os.listdir(path_subs):
            ...
            for sp in ["scores", "patterns"]:
                good_files["times"] = op.join(path_subs, 'VME_S01', sp,
                                              f"{sp}_per_sub__times.npy")
                good_files["freqs"] = op.join(path_subs, 'VME_S01', sp,
                                              f"{sp}_per_sub__freqs.npy")
                path_sp = op.join(path_sub, sp)
                for tf in ["times", "freqs"]:
                    good_file = op.join(path_subs, 'VME_S01', sp,
                                        f"{sp}_per_sub__{tf}.npy")
