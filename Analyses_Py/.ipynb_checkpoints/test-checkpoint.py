...
fig = plt.figure()
plt.plot(eposLeft.times, eposRight.copy().pick_channels(chans_CDA[0]).get_data().mean(0).mean(0))
plt.plot(eposLeft.times, eposRight.copy().pick_channels(chans_CDA[1]).get_data().mean(0).mean(0))
...
eposCDA = eposContra.copy()
eposCDA._data = eposContra._data - eposIpsi._data

eposHi = eposCDA[event_dict['LoadHigh']]
eposLo = eposCDA[event_dict['LoadLow']]

eposS = eposCDA[event_dict['EccS']]
eposM = eposCDA[event_dict['EccM']]
eposL = eposCDA[event_dict['EccL']]

eposHiS = eposHi[event_dict['EccS']]
eposHiM = eposHi[event_dict['EccM']]
eposHiL = eposHi[event_dict['EccL']]

eposLoS = eposLo[event_dict['EccS']]
eposLoM = eposLo[event_dict['EccM']]
eposLoL = eposLo[event_dict['EccL']]

evoHi = eposHi.average()
evoLo = eposLo.average()

evoS = eposS.average()
evoM = eposM.average()
evoL = eposL.average()

evoHiS = eposHiS.average()
evoHiM = eposHiM.average()
evoHiL = eposHiL.average()

evoLoS = eposLoS.average()
evoLoM = eposLoM.average()
evoLoL = eposLoL.average()

mne.viz.plot_compare_evokeds(dict(High = evoHi, Low=evoLo), combine='mean', vlines=[-0.8, 0])
mne.viz.plot_compare_evokeds(dict(Small = evoHiS, Medium=evoHiM, Large=evoHiL), combine='mean', vlines=[-0.5, 0])
mne.viz.plot_compare_evokeds(dict(High = evoHiM, Low=evoLoM), combine='mean', vlines=[-0.8, 0])





evoCDA = eposCDA.average()

fig = plt.figure()
plt.plot(eposCDA.times, eposCDA.copy().get_data().mean(0).mean(0))
plt.axhline(color='black', 
                lw = 0.5, 
                ls = ':')
plt.axvline(x=-0.8, color='black', ls=':')
...
mne.viz.plot_compare_evokeds(dict(contra = contra, ipsi=ipsi), combine='mean', vlines=[-0.8, 0], show_legend=['haha', 'hehe'])
...
powSum._data = powerC.copy()._data  + powerI._data
...
cont = powerC.plot(baseline=(-.3, -0), picks='all', mode=None, tmin=-1.3, tmax=2, title='contralateral', vmax=0.5, vmin=-0.5)
tot = powDiff.plot(baseline=None, picks='all', mode='logratio', tmin=-1.3, tmax=2, title='contra-ipsi', cmap='RdBu')
ips.show()
cont.show()
...
tot.show()
alpCo.plot
mne.viz.plot_compare_evokeds([alpCoEv, alpIpEv], combine='mean', vlines=[-0.5, 0])
 """