...
def decode(sub_list_str, conditions, event_dict, reps = 1, shuffle_labels = False, save_scores = True, save_csp_patterns = True, 
    ...
    for subID in sub_list_str:
        ...
        if (save_scores or save_csp_patterns):
            ...
            fpath = op.join(config.path_decod_tfr, part_epo, signaltype, contrast_str, shuf_labs, sub_folder)
...
#res_load = decode([subID], ['LoadLow', 'LoadHigh'], config.event_dict, reps=5, shuffle_labels=True,
#                  overwrite=True)
_ = decode(sub_list_str, ['LoadLowEccL', 'LoadHighEccL'], config.event_dict, reps=5, shuffle_labels=True, overwrite=True)
_ = decode(sub_list_str, ['LoadLowEccS', 'LoadHighEccS'], config.event_dict, reps=5, shuffle_labels=True, overwrite=True)
_ = decode(sub_list_str, ['LoadLowEccM', 'LoadHighEccM'], config.event_dict, reps=5, shuffle_labels=True, overwrite=True)
_ = decode(sub_list_str, ['EccS', 'EccL'], config.event_dict, reps=5, shuffle_labels=True, overwrite=True)
_ = decode(sub_list_str, ['EccM', 'EccL'], config.event_dict, reps=5, shuffle_labels=True, overwrite=True)
_ = decode(sub_list_str, ['EccS', 'EccM'], config.event_dict, reps=5, shuffle_labels=True, overwrite=True)
