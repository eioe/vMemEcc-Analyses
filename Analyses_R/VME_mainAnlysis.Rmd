...
## Requirements
We ran these analyses using `r R.Version()$version.string` (in RStudio `r rstudioapi::versionInfo()$version`). 
...
summary_rem_trials_per_sub <- exp_data %>% 
  group_by(ppid) %>% 
...
stats_memLoad <- list(
  name = "StimN", 
  printName = "memLoad", 
  levels = str_c("StimN_", unique(exp_data$c_StimN), '_'),
  stats = acc_per_memLoad
)


# Summary per Ecc:
acc_per_ecc <- exp_data %>% 
  group_by(ppid, c_Ecc) %>% 
  summarise(perc_resp_corr = mean(c_ResponseCorrect) * 100,
            .groups = "drop") %>% 
...
stats_ecc <- list(
  name = "ecc", 
  printName = "Ecc", 
  levels = str_c("Ecc_", sort(unique(exp_data$c_Ecc)), '_'), 
  stats = acc_per_ecc
)


for (tmp in list(stats_overall, stats_memLoad, stats_ecc)) {
...
for (tmp in list(stats_overall, stats_memLoad, stats_ecc)) {
  for (lvl in tmp$levels) {
    for (stat in c("mean", "sd", "min", "max")) {
      extract_var(str_glue("mem_acc_{lvl}{stat}"), tmp$stats[str_c(lvl, stat)], exp_format="%.2f")
    }
  }
}
