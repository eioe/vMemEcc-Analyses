...
func_analysis_11 <- function() {
  ...
  c1.aov <- data_behav %>% 
    ...
    summarise(mean_apwr_retent = mean(alphapwr_diff_retent, na.rm = T)) %>% 
    ungroup() %>% 
    select("mean_apwr_retent", "c_StimN", "c_Ecc", "ppid") 
  
  c1.reduced <- c1.aov %>% 
    pivot_wider(names_from = c(c_StimN, c_Ecc), 
                values_from = mean_apwr_retent, 
                names_prefix = 'cond_') %>% 
  ...
  #--------------------------------------------------------------------------
  ## Run ANOVA
  aov.srt <- aov(mean_apwr_retent ~ c_StimN*c_Ecc + Error(ppid/(c_StimN* c_Ecc)),data=c1.aov)
  ...
  res_ttest <- c1.aov %>% 
    pairwise_t_test(
      mean_apwr_retent ~ c_Ecc, paired = TRUE, 
  ...
  c1.plt <- c1.aov %>% 
    ...
    summarise(mean_apwr_retent = mean(mean_apwr_retent, na.rm = T)) %>% 
  ...
  figa <- figa + mytheme
  figa <- figa + ylab("mean lateralized alpha power (uV^2)") + xlab("Eccentricity")
