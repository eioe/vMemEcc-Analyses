...
sumstat <- fulldat %>% 
  filter(BlockStyle %in% c('perception')) %>% 
...
colnames(meanamp_dat) <- c('ppid', 'c_StimN', 'c_Ecc', 'mean_amp')

sumstat_ma <- meanamp_dat %>% 
  group_by(c_StimN, c_Ecc, ppid) %>% 
  summarise(mean_ma = mean(mean_amp))
...
colnames(apwr_dat) <- c('ppid', 'c_StimN', 'c_Ecc', 'mean_alpha')
