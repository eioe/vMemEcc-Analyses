...
# skip trainings blocks:
blocks <- blocks[!blocks %in% c('Block1', 'Block3', 'R_plots')]
...
for (block_nr in blocks) {
  ...
  if ((file.exists(fname_plot) && !overwrite_existing) && !suppress_plotting) {
  ...
  data_gaze <- data_gaze %>% 
    ...
    # Calculate vert. and hor. difference between gaze vector and vector from eye to fix cross:
    # TODO: replace dev0/1_deg_x/y with gaze_dev 
    ...
    for (trial in 1:nrow(data_stimon)) {
      ...
      data_fix <-  data_gaze %>% 
        ...
        rowwise() %>% 
        mutate(blink = if_else(any(abs(gaze_timestamp - blink_times) < timings$buffer_blink), TRUE, FALSE)) %>% 
        ungroup()
      ...
      plt <- data_fix %>% 
        ...
        geom_line(data = data_pupils %>%  
                  ...
                  aes(x = pupil_timestamp, y = rollmean(confidence, 10, na.pad = T) + ymin_conf_subplt)) +
        ...
        # arrow indicationg cue direction:
    ...
    ui_info(glue('Finishing {block_nr}. 
                   Running for {time_elapsed} seconds.  
                   Approx. {time_remain} seconds remaining.'))
