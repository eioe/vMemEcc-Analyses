...
for (task in c('perc', 'exp')) {
  ...
  for (outvar in outvars) {
    ...
    ## Pairwise differences of the mean (with CIs):
    ...
    meanDiff <- mean(tmp_df$StimN_2 - tmp_df$StimN_4)
    ci_diff <- ci95_meandiff(tmp_df$StimN_2, tmp_df$StimN_4)
    print("Difference between Memory Load conditions:")
    print(str_glue("  Mean (CI):     {format(meanDiff, digits=3)} ({format(ci_diff$lower, digits=3)} - {format(ci_diff$upper, digits=3)})"))
    
    extract_var(str_glue("{dv_str}_{task}_delta_StimN2vsStimN4_mean"), meanDiff, exp_format="%.2f")
    extract_var(str_glue("{dv_str}_{task}_delta_StimN2vsStimN4_cilower"), ci_diff$lower, exp_format="%.2f")
    extract_var(str_glue("{dv_str}_{task}_delta_StimN2vsStimN4_ciupper"), ci_diff$upper, exp_format="%.2f")
    ...
    for (cp in contrast_pairs) {
      ...
      meanDiff <- mean(tmp_df[[str_glue("Ecc_{cp[1]}")]] - tmp_df[[str_glue("Ecc_{cp[2]}")]])
      ci_diff <- ci95_meandiff(tmp_df[[str_glue("Ecc_{cp[1]}")]], tmp_df[[str_glue("Ecc_{cp[2]}")]])
      print(str_glue("  Contrast: {cp[1]} - {cp[2]}"))
      print(str_glue("    Mean (CI):     {format(meanDiff, digits=3)} ({format(ci_diff$lower, digits=3)} - {format(ci_diff$upper, digits=3)})"))
      
      extract_var(str_glue("{dv_str}_{task}_delta_Ecc{cp[1]}vsEcc{cp[2]}_mean"), meanDiff, exp_format="%.2f")
      extract_var(str_glue("{dv_str}_{task}_delta_Ecc{cp[1]}vsEcc{cp[2]}_cilower"), ci_diff$lower, exp_format="%.2f")
      extract_var(str_glue("{dv_str}_{task}_delta_Ecc{cp[1]}vsEcc{cp[2]}_ciupper"), ci_diff$upper, exp_format="%.2f")
    }
